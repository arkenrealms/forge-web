...
const customizeRequiredMark: any = (
  ...
  const onFinish = async (values: any, isDraft = false) => {
    ...
    function cleanData(d: any) {
      for (const key in d) {
        ...
        if (key === 'data') {
          console.log('cdcd', d)
          const originalData = _.keyBy(
            (componentsByKey[d.key] as any).__original_data,
            primaryKey
          ) as any
          const newData = []
            ...
            onClick={() => {
              for (let i = 0; i < component.gridColumns; i++) {
                const repeatedComponent = JSON.parse(
                  JSON.stringify(
                    component.components[component.components.length - component.gridColumns]
                  )
                )

                const counter =
                  (parseInt(repeatedComponent.key.match(/(\d+)$/)?.[0] || '0', 10) ||
                    component.gridRows) + 1
                ...
                for (const repeatedComponentComponentKey of Object.keys(
                  repeatedComponentComponents
                )) {
                  const repeatedComponentComponent =
                    repeatedComponentComponents[repeatedComponentComponentKey]

                  repeatedComponentComponent.key =
                    repeatedComponentComponent.key.replace(/(\d+)$/, '') + counter

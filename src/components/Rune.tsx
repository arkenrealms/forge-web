...
import { Button, Flex, Skeleton, BaseLayout, Card, CardBody, Text, LinkExternal, AutoRenewIcon, Heading } from '~/ui';
...
import items from '@arken/node/legacy/data/items';
...
const Rune = ({ match }) => {
  ...
  const { data: token } = trpc.seer.chain.getChainToken.useQuery<Arken.Chain.Types.ChainToken>({
    where: {
      symbol: { equals: id },
    },
  });
      ...
      <Cards>
          ...
          <ItemCard>
            <InventoryWrapper>
              <Inventory
                ...
                noDisabled
                hideCategories
                defaultItemSelected={`inventory${parseInt(token?.meta?.id) - 1}`}
        ...
        <VerticalCards>
          <ItemCard>
            ...
            <br />
            {token?.meta?.category === ItemCategoriesType.RUNE ? (
              <>
                {token?.meta?.tokenAddress ? (
                  <div>
                    <Text mr="10px">{t('Contract: ')}</Text>
                    <LinkExternal href={`https://bscscan.com/address/${token?.meta?.tokenAddress}`}>
                      {token?.meta?.tokenAddress}
                    </LinkExternal>
                    <br />
                    <Text mr="10px">{t('Chart: ')}</Text>
                    <LinkExternal href={`https://dex.guru/token/${token?.meta?.tokenAddress}-bsc`}>
                      Dex.Guru
                    </LinkExternal>
                  </div>
                ) : (
            ...
            <br />
            {token?.meta?.details
              ? Object.keys(token?.meta?.details).map((key) => (
                  <div key={key}>
                    <Text mr="10px" bold>
                      {t(key)}:
                    </Text>{' '}
                    <Text>{token?.meta?.details[key]}</Text>
                    <br />
                  </div>
                ))
              : null}
            {token?.meta?.branches
              ? Object.keys(token?.meta?.branches).map((i) => (
                  <div key={i}>
                    <Text mr="10px" bold>
                      {GameTitleMap[i]} Utility:
                    </Text>{' '}
                    <Text>{token?.meta?.branches[i].description || 'Not announced yet.'}</Text>
                    <br />
                  </div>
                ))
              : null}

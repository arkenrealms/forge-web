...
import { JSBI, Percent, Router, SwapParameters, Trade, TradeType } from '@arcanefinance/sdk'
import { useMemo } from 'react'
import useWeb3 from '~/hooks/useWeb3'
import { BIPS_BASE, DEFAULT_DEADLINE_FROM_NOW, INITIAL_ALLOWED_SLIPPAGE } from '~/constants'
import { useTransactionAdder } from '~/state/transactions/hooks'
import { calculateGasMargin, getRouterContract, getRouterV2Contract, isAddress, shortenAddress } from '~/utils/web3'
...
function useSwapCallArguments(
  ...
  return useMemo(() => {
    ...
    const contract: Contract | null =
      trade.route.input.symbol === 'RXS' || trade.route.output.symbol === 'RXS'
      ...
      callback: async function onSwap(): Promise<string> {
        ...
        return contract[methodName](...args, {
          ...
          .catch((error: any) => {
            ...
            if (error?.code === 4001) {
              console.error(`Transaction rejected`, error, methodName, args, value)
            } else {

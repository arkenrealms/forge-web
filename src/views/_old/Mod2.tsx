...
const Evolution: React.FC<any> = () => {
  ...
  const fetchState = async (_realm) => {
    ...
    if (res2.status === 1) {
      for (const server of res2.state.servers) {
        if (!server.info?.connectedPlayers) continue;
  ...
  const hideMinimap = async () => {
    await sendRealmWebRequest(realm, 'RS_HideAllMinimapRequest', { sceneId: 0 });
  };

  const showMinimap = async () => {
    await sendRealmWebRequest(realm, 'RS_ShowAllMinimapRequest', { sceneId: 0 });
  };
    ...
    <Page>
      ...
      <Cards>
        ...
        <MainCard style={{ overflow: 'visible' }}>
          ...
          {sockets[realm?.key]?.connected ? <p>Not authorized</p> : null}
          {sockets[realm?.key]?.connected ? (
            <>
              {realmInfo ? (
                <>
                  <p>
              ...
              <br />
              <br />
              <Button scale="sm" onClick={hideMinimap}>
                Hide Minimap
              </Button>
              <br />
              <br />
              <Button scale="sm" onClick={showMinimap}>
                Show Minimap
              </Button>
              <br />
              <br />
